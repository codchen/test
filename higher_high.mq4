...
void OnTick() {
  ...
  if (HasHigherHigh() && HasHigherLow() && last_tentative_high < last_tentative_low) {
  	string name = "up_" + Bars;
  	ObjectCreate(name, OBJ_TEXT, 0, Time[1], Close[1]);
  	ObjectSetText(name, CharToStr(159), 14, "Wingdings", Purple);
  }
  if (HasLowerHigh() && HasLowerLow() && last_tentative_low < last_tentative_high) {
  	string name = "down_" + Bars;
  	ObjectCreate(name, OBJ_TEXT, 0, Time[1], Close[1]);
...
bool HasHigherHigh() {
	if (last_tentative_high != -1) {
		return High[last_tentative_high] > High[last_ten_highs[0]] + threshold * iATR(NULL, tf, 14, last_tentative_high);
	} else {
		return High[last_ten_highs[0]] > High[last_ten_highs[1]] + threshold * iATR(NULL, tf, 14, last_ten_highs[0]);
	}
}

bool HasHigherLow() {
	if (last_tentative_low != -1) {
		return Low[last_tentative_low] > Low[last_ten_lows[0]] + threshold * iATR(NULL, tf, 14, last_tentative_low);
	} else {
		return Low[last_ten_lows[0]] > Low[last_ten_lows[1]] + threshold * iATR(NULL, tf, 14, last_ten_lows[0]);
	}
}
